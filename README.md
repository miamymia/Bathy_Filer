...
- `Create_dirs.py` creates folder structure for any cruise folder that doesn't have this structure yet. Ignore those that do have the structure. Usage: _"python Create_dirs.py /path/to/local/dst_dir/ XX"_ (XX = vessel abbr., e.g. "SO" for SONNE, then do: _python Create_dirs.py /path/to/local/folder/ SO_)
